...
export var RANDOM_SHAKE_STRENGTH: float = 60.0
# Multiplier for lerping the shake strength to zero
export var SHAKE_DECAY_RATE: float = 2.0
...
onready var text = $GearLevelText
...
onready var text_starting_rect = text.rect_position
...
#	var anim_speed = 0.25
	if new_gear_level > current_gear_level:
		if current_gear_level != -1:
			anim_speed = 1.5
			apply_noise_shake()
		$Tween.interpolate_property($GearLevelBar, "value", old_value, 1000, anim_speed, Tween.TRANS_CUBIC, Tween.EASE_OUT)
...
func _process(delta: float) -> void:
	...
	text.rect_position = text_starting_rect + shake_offset
